...
CL = "/home/user/yuriybuz/mechanic-checklist/index.html"   # оригінальний клієнт: довідник не має їхати за правками фронтенду
ROWS = "/tmp/claude-0/-home-user-Spare-parts-mechanical-service/63268d39-83d9-5eb2-8c17-fa439b3aba43/scratchpad/answers.json"
OUT = "/home/user/mechanic-checklist"
...
# ---------- 5. чек-лист майстра ----------
# Ідентифікатори й назви груп звірені з живим фронтендом майстра
# (YuriyBuz/shift-checklist, check-list-master.html): усі 30 пунктів збіглися
# за текстом і за стадією. Беремо його ід, а не власні — інакше клієнт
# і довідник говорили б про одні й ті самі пункти різними іменами.
MASTER = [
    ("start", "s1", "start-1", "1. Прийом зміни та перевірка персоналу", [
    ...
    ("start", "s2", "start-2", "2. Контроль санітарного стану та обладнання", [
    ...
    ("start", "s3", "start-3", "3. Організація виробничого процесу", [
    ...
    ("start", "s4", "start-4", "4. Звітність", [
        "Доповісти керівництву про початок виробничого процесу."]),
    ("end", "e1", "end-1", "1. Завершення виробничих операцій", [
    ...
    ("end", "e2", "end-2", "2. Підготовка до наступної зміни", [
    ...
    ("end", "e3", "end-3", "3. Звітність та передача зміни", [
    ...
    ("end", "e4", "end-4", "4. Завершальні перевірки та безпека", [
    ...
    ("end", "e5", "end-5", "5. Комунікація та підбиття підсумків", [
...
master_texts = {t for _, _, _, _, ts in MASTER for t in ts}
...
for stage, gid, group_id, title, texts in MASTER:
    for si, t in enumerate(texts, 1):
        iid = f"master.{gid}-{si}"
        items.append({"item_id": iid, "role": "Майстер", "group_id": group_id, "group_title": title,
                      ...
                      "notes": "звірено з фронтендом майстра"})
